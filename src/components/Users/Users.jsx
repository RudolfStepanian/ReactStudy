import React from "react";
import cl from './Users.module.css';
import DialogItem from "../Dialogs/DialogItem/DialogItem";

let Users = (props) => {
    if(props.users.length == 0){
        props.setUsers([
            {
                id: 1,
                photoUrl: 'data:image/png;base64,',
                followed: false,
                fullName: 'Johnatan',
                status: 'I am ok',
                location: {city: 'city19', country: 'country7'}
            },
            {
                id: 2,
                photoUrl: 'https://previews.123rf.com/images/triken/triken1608/triken160800029/61320775-male-avatar-profile-picture-default-user-avatar-guest-avatar-simply-human-head-vector-illustration-i.jpg',
                followed: true,
                fullName: 'Dmitry',
                status: 'I am not ok',
                location: {city: 'city15', country: 'country8'}
            },
            {
                id: 3,
                photoUrl: 'https://cdn2.iconfinder.com/data/icons/website-icons/512/User_Avatar-512.png',
                followed: false,
                fullName: 'Sam',
                status: 'I am high',
                location: {city: 'city51', country: 'country1'}
            },
            {
                id: 4,
                photoUrl: 'https://cdn3.iconfinder.com/data/icons/business-round-flat-vol-1-1/36/user_account_profile_avatar_person_student_male-512.png',
                followed: true,
                fullName: 'Nick',
                status: 'ok',
                location: {city: 'city02', country: 'country4'}
            },
            {
                id: 5,
                photoUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQ_db8JezfIf1GA-hmcA99e-22bj0ksOr2P31u_k2a9l1QRgQ3T',
                followed: true,
                fullName: 'Anna',
                status: 'jjjj',
                location: {city: 'city46', country: 'country3'}
            },
            {
                id: 6,
                photoUrl: 'https://cdn1.iconfinder.com/data/icons/user-pictures/100/female1-512.png',
                followed: false,
                fullName: 'Nina',
                status: 'nnnn',
                location: {city: 'city1', country: 'country7'}
            }
        ]);
    }

    return <div>
        {props.users.map(u => <div key={u.id}>
                <span>
                    <div>
                        <img src={u.photoUrl} className={cl.usersPhoto} alt=""/>
                    </div>
                    <div>
                        {u.followed
                            ? <button onClick={() => {
                                props.unfollow(u.id)
                            }}>unfollow</button>
                            : <button onClick={() => {
                                props.follow(u.id)
                            }}>follow</button>}
                    </div>
                </span>
            <span>
                    <span>
                        <div>{u.fullName}</div><div>{u.status}</div>
                    </span>
                    <span>
                        <div>{u.location.city}</div><div>{u.location.country}</div>
                    </span>
                </span>
        </div>)}
    </div>
};

export default Users;